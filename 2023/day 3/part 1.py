...
if __name__ == "__main__":
    ...
    for row in range(len(engine)):
        column = 0
        while column < len(engine[row]):
        #for column in range(len(sliced[row])):
            hasAdjacentSymbol:bool = False
            ...
            if not engine[row][column].isdigit():
                ...
                # finding the start and end of the number
                counter = 0
                while column + counter < len(engine[row]) and engine[row][column + counter].isdigit():
                        counter += 1
                
                counter -= 1 if column < len(engine[row]) else 0 # remove only if not at the end - not went to far
                num_end = column + counter

            num = int(engine[row][column:num_end + 1])


            # looking for adjacent symbols around the number

            # checking before and after num if available
            symbolBeforeNum:bool = isSymbol(engine[row][column - 1 if column > 0 else 0])
            tmp = engine[row][num_end + 1 if (num_end < (len(engine[row]) - 1) ) else num_end]
            symbolAfterNum:bool = isSymbol(tmp)
            hasAdjacentSymbol = symbolBeforeNum or symbolAfterNum

            
            # above and under the number and also diagonals
            if not hasAdjacentSymbol:
                start = column - 1 if column > 0 else 0
                end = num_end + 1 if num_end < len(engine[row]) - 1 else num_end
                if row > 0: # checking above and above diagonal
                    for i in range(start, end + 1):
                        if isSymbol(engine[row - 1][i]):
                            hasAdjacentSymbol = True
                            break
                if not hasAdjacentSymbol: # skipping if found previously
                    if row < len(engine) - 1: # checking under and under diagonal
                        for i in range(start, end + 1):
                            if isSymbol(engine[row + 1][i]):
                                hasAdjacentSymbol = True
                                break
            ...
            column += num_end - column + 1 # skipping number and going to the char after it
