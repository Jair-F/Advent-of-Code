...
def isGear(char:str) -> bool:
    return char == '*'

"""
    insert a one dimensional list and one position in a in the number
    and the function returns the number extracted
"""
def getNumber(pos, engine:list) -> int:
    if not engine[pos].isdigit():
        raise Exception("invalid argument - position not in number")
    
    numStart = None
    numEnd = None
    numSignNegative:bool = False # true for -, false for +

    counter = 0
    while pos - counter > 0:
        if engine[pos - counter - 1].isdigit(): # peeking if the next pos is still a digti
            counter += 1
        else:
            break
    numStart = pos - counter
    
    counter = 0

    while pos + counter < len(engine) - 1:
        if engine[pos + counter + 1].isdigit():
            counter += 1
        else:
            break
    numEnd = pos + counter
    ...
    if numStart - 1 >= 0:
        numSignNegative = engine[numStart - 1] == '-'

    number = int(engine[numStart:numEnd + 1])
...
if __name__ == "__main__":
    ...
    for row in range(len(engine)):
        for column in range(len(engine[row])):
            if isGear(engine[row][column]):
                gearRatio = 1
                # checking the positions around
                checkStart:int = column - 1 if column > 0 else column # column
                checkEnd:int = column + 1 if column < len(engine[row]) - 1 else column # column
                partNumberCounter = 0

                if column > 0: # checking before
                    if engine[row][column - 1].isdigit(): # we have a number before
                        partNumberCounter += 1
                        gearRatio *= getNumber(column - 1, engine[row])
                if column < len(engine[row]) - 1: # checking after
                    if engine[row][column + 1].isdigit(): # we have a number after
                        partNumberCounter += 1
                        gearRatio *= getNumber(column + 1, engine[row])

                if partNumberCounter < 2:
                    # checking above and under
                    if row > 0:
                        if engine[row - 1][column].isdigit(): # we have only one number above
                            partNumberCounter += 1
                            gearRatio *= getNumber(column, engine[row - 1])
                        else: # we could have up to two numbers above
                            if engine[row - 1][checkStart].isdigit(): # above left
                                partNumberCounter += 1
                                gearRatio *= getNumber(checkStart, engine[row - 1])
                            if engine[row - 1][checkEnd].isdigit(): # above right
                                partNumberCounter += 1
                                gearRatio *= getNumber(checkEnd, engine[row - 1])
                            
                        if partNumberCounter > 2:
                            continue # more than two part numbers we skipp

                    if row < len(engine) - 1: 
                        if engine[row + 1][column].isdigit(): # we have only one number under
                            partNumberCounter += 1
                            gearRatio *= getNumber(column, engine[row + 1])
                        else: # we could have up to two numbers under
                            if engine[row + 1][checkStart].isdigit(): # below left
                                partNumberCounter += 1
                                gearRatio *= getNumber(checkStart, engine[row + 1])
                            if engine[row + 1][checkEnd].isdigit(): # below right
                                partNumberCounter += 1
                                gearRatio *= getNumber(checkEnd, engine[row + 1])
