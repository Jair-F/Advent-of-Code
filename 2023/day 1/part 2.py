...
if __name__ == "__main__":
    input = input.split() # by newlines and whitecharacters
    calibaration_values = []
    ...
    for line in input:
        ...
        first_found_pos:int = len(line)
        last_found_pos:int = 0

        # finding first num digit
        ...
        # finding first word digit
        for word, num in matcher.items():
            pos = line.find(word) # searching until the first found num digit
            if pos != -1 and pos < first_found_pos:
                first_found_pos = pos
                first_digit = num
                #if pos == 0:
                #    break

        # finding last word digit
        for word, num in matcher.items():
            pos = line.rfind(word) # searching from the last found num digit to the end
            if pos != -1 and pos > last_found_pos:
                last_found_pos = pos
                last_digit = num

        #print(f":\t firstDigit: {first_digit}, lastDigit: {last_digit} -> {first_digit+last_digit}")
        calibaration_values.append(int(first_digit + last_digit))
    
    sum:int = 0
    #print(calibaration_values)
    for value in calibaration_values:
        sum += value
