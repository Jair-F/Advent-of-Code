...
int main() {
    ...
    Board *last_win_board = nullptr;
    unsigned int winner_board_score = 0;

    //input.open("example.txt", std::fstream::in);
    ...
    for(unsigned short r_num : random_nums) {
        if(last_win_board != nullptr) {   // we found the winnerboard - exit
            break;
        }
        ...
        for(auto iterator = boards.begin(); iterator != boards.end(); ++iterator) {
            if(check_board(*iterator)) {
                if(boards.size() == 1) {
                    last_win_board = &boards[0];
                    break;
                }
    ...
    for(unsigned short row = 0; row < last_win_board->marked_nums.size(); ++row) {
        for(unsigned short column = 0; column < last_win_board->marked_nums[row].size(); ++ column) {
            if(last_win_board->marked_nums[row][column] == false) {
                sum_unmarked_nums += last_win_board->numbers[row][column];
            }
        }
    }
    winner_board_score = sum_unmarked_nums * last_called_number;
    std::cout << "Sum of unmarked numbers: " << sum_unmarked_nums << '\n';
    std::cout << "Last called number: " << last_called_number << '\n';
    std::cout << "Winner-Board-Score: " << winner_board_score << '\n';

    for(unsigned short x = 0; x < last_win_board->numbers.size(); ++x) {
        for(unsigned short y = 0; y < last_win_board->numbers[y].size(); ++y) {
            if(last_win_board->marked_nums[x][y] == false) {
                std::cout << last_win_board->numbers[x][y] << "  ";
            }
            else {
                std::cout << "  ";
            }
    ...
    for(unsigned short x = 0; x < last_win_board->numbers.size(); ++x) {
        for(unsigned short y = 0; y < last_win_board->numbers[x].size(); ++y) {
            std::cout << last_win_board->marked_nums[x][y] << ' ';
        }
        std::cout << '\n';
    }
    std::cout << "\n\n";
